...
*/
// What each job actually involves, in a sentence, so the invitation says
// what is being asked of somebody rather than just naming a role.
const ROLE_DUTIES = {
  manager: "handle customer orders - see them come in and move them from packing to delivered",
  admin: "manage the catalogue and handle customer orders",
};

const adminInviteEmail = ({ email, code, expiresInMinutes, role = "admin", invitedBy, shopUrl }) => ({
  subject: `You have been invited to help run ShopNGo`,
  html: layout(
    `You have been invited as ${role === "manager" ? "a manager" : "an admin"}`,
...
const adminGrantedEmail = ({ name, role = "admin", invitedBy, shopUrl }) => ({
  subject: "You now have back office access to ShopNGo",
  html: layout(
